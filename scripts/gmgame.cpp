...
SCRIPTS_API bool MapExtraKillMessages = false;
...
void SSGMGameManager::ReadMapConfig()
{
	...
	if (SSGMIni)
	{
		...
		MapExtraKillMessages = SSGMIni->Get_Bool(s,"ExtraKillMessages",ExtraKillMessages);
		delete[] s;
	}
	else
	{
...
int SSGMGameManager::PowerupPurchaseHook(BaseControllerClass *base,GameObject *purchaser,unsigned int cost,unsigned int preset,const char *data)
{
	...
	SSGMGameLog::Log_Gamelog("PURCHASED;POWERUP;%s;%s;%s", str3, Get_Definition_Name(preset), str2);
	delete[] str3;
	delete[] str2;
...
int SSGMGameManager::VehiclePurchaseHook(BaseControllerClass *base,GameObject *purchaser,unsigned int cost,unsigned int preset,const char *data)
{
	...
	str.Format("Purchase: %s - %s",str3,str2);
	SSGMGameLog::Log_Message(str,"_PURCHASE");
	SSGMGameLog::Log_Gamelog("PURCHASED;VEHICLE;%s;%s;%s", str3, Get_Definition_Name(preset), str2);
	delete[] str3;
	delete[] str2;
	bool fp = FreePurchases;
...
int SSGMGameManager::CharacterPurchaseHook(BaseControllerClass *base,GameObject *purchaser,unsigned int cost,unsigned int preset,const char *data)
{
	...
	str.Format("Purchase: %s - %s",str3,str2);
	SSGMGameLog::Log_Message(str,"_PURCHASE");
	SSGMGameLog::Log_Gamelog("PURCHASED;CHARACTER;%s;%s;%s", str3, Get_Definition_Name(preset), str2);
	delete[] str3;
	delete[] str2;
	bool fp = FreePurchases;
...
void SSGMGameManager::DialogHook(int PlayerID, int DialogID, int ControlID, DialogMessageType MessageType) 
{
	for (int i = 0; i < RegisteredEvents[EVENT_DIALOG_HOOK].Count(); i++)
	{
		RegisteredEvents[EVENT_DIALOG_HOOK][i]->OnDialog(PlayerID,DialogID,ControlID,MessageType);
	}
}
