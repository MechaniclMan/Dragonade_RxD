	...
	if (veh)
	{
		...
		if ((!scr->OwnerID) || (!Commands->Find_Object(scr->OwnerID)))
		{
			if (Get_Vehicle_Driver(veh) == obj)
			{
				...
				if (!Is_Script_Attached(veh, "MS_AI_Marker_Vehicle_Search_Ignore"))
				{
					StringClass scriptParams;
					scriptParams.Format("%d", Get_Object_Type(veh));
					Commands->Attach_Script(veh, "MS_AI_Marker_Vehicle_Search_Ignore", scriptParams.Peek_Buffer());
				}
			}
			else
			{
...
SCRIPTS_API void Lock_Vehicle(GameObject *obj)
{
	...
	if (boundveh)
	{
		if (!boundveh->Get_Owner())
		{
			...
			if (!Is_Script_Attached(boundveh, "MS_AI_Marker_Vehicle_Search_Ignore"))
			{
				StringClass scriptParams;
				scriptParams.Format("%d", Get_Object_Type(boundveh));
				Commands->Attach_Script(boundveh, "MS_AI_Marker_Vehicle_Search_Ignore", scriptParams.Peek_Buffer());
			}
		}
		else
		{
...
void SSGM_Vehicle::Killed(GameObject *obj,GameObject *killer)
{
	...
	if (Get_Vehicle_Mode(obj) == VEHICLE_TYPE_TURRET)
	{
		Log_Killed_Message(obj, killer, "BUILDING");
	}
	else
	{
		Log_Killed_Message(obj, killer, "VEHICLE");
	}
}
